 ...
 * +----+-----------+-------------------------------------+
 */
static bool disas_uncond_b_imm(branch_type_t *s, uint32_t opcode)
{
 ...
 * +----+-------------+----+---------------------+--------+
 */
static bool disas_comp_b_imm(branch_type_t *s, uint32_t opcode)
{
        // reset_btype(s);
 ...
 * +----+-------------+----+-------+-------------+------+
 */
static bool disas_test_b_imm(branch_type_t *s, uint32_t opcode)
{
 ...
 * +---------------+----+---------------------+----+------+
 */
static bool disas_cond_b_imm(branch_type_t *s, uint32_t opcode)
{
...
/* CLREX, DSB, DMB, ISB */
static bool handle_sync(branch_type_t *s, uint32_t opcode, unsigned int op1,
 ...
 * +---------------------+---+-----+-----+-------+-------+-----+------+
 */
static bool disas_system(branch_type_t *s, uint32_t opcode)
{
        // printf("disas_system\n");
 ...
 * +---------------+-------+-------+-------+------+-------+
 */
static bool disas_uncond_b_reg(branch_type_t *s, uint32_t opcode)
{
...
static bool disas_b_exc_sys(branch_type_t *s, uint32_t opcode)
{
...
bool decode_armv8_branch_opcode(branch_type_t *s, uint32_t opcode)
{
        *s = QEMU_Non_Branch;
