...
static bool libqflex_flexus_init(void)
{
        ...
        void *handle = NULL;
        if ((handle = dlopen(qemu_libqflex_state.lib_path, RTLD_LAZY)) ==
            NULL) {
                error_report("ERROR: while opening %s => %s",
                             qemu_libqflex_state.lib_path, dlerror());
                return false;
        }

        FLEXUS_INIT_t flexus = NULL;
        if ((flexus = (FLEXUS_INIT_t)(dlsym(handle, "flexus_init"))) == NULL) {
                error_report("ERROR: cannot find 'flexus_init' in %s: %s",
                             qemu_libqflex_state.lib_path, dlerror());
                return false;
        }
        ...
        flexus(&qemu_api, &flexus_api, qemu_libqflex_state.n_vcpus,
               qemu_libqflex_state.cfg_path, qemu_libqflex_state.debug_lvl,
               nb_cycles->str,
               "." // CWD
        );
...
{
        QemuOpts *opts =
            qemu_opts_parse_noisily(qemu_find_opts("libqflex"), optarg, false);
        ...
        uint32_t const cycles_mask =
            qemu_opt_get_number(opts, "cycles-mask", 1);

        qemu_libqflex_state.cycles = cycles;
        qemu_libqflex_state.cycles_mask = cycles_mask;

        if (lib_path)
                qemu_libqflex_state.lib_path = strdup(lib_path);
        if (cfg_path)
                qemu_libqflex_state.cfg_path = strdup(cfg_path);
        if (debug_lvl)
                qemu_libqflex_state.debug_lvl = strdup(debug_lvl);
        if (ckpt_path)
                qemu_libqflex_state.ckpt_path = strdup(ckpt_path);

        if (mode) {
                if (strcmp(strdup(mode), "trace") == 0)
                        qemu_libqflex_state.mode = MODE_TRACE;
                if (strcmp(strdup(mode), "timing") == 0)
