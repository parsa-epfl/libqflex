 ...
 *
 */
static inline void assert_index_in_range(size_t idx, size_t min, size_t max)
{
 ...
 */
static inline vCPU_t *lookup_vcpu(size_t idx)
{
...
uint64_t libqflex_read_register(size_t cpu_index, register_type_t reg_type,
                                size_t idx)
{
        ...
        case FLOATING_POINT:
                 ...
                 *
                 * Note that the mapping between S, D, and Q views of the
                 * register bank differs between AArch64 and AArch32. In
                 * AArch32: Qn = regs[n].d[1]:regs[n].d[0] Dn = regs[n / 2].d[n
                 * & 1] Sn = regs[n / 4].d[n % 4 / 2], bits 31..0 for even n,
                 * and bits 63..32 for odd n (and regs[16] to regs[31] are
                 * inaccessible) In AArch64: Zn = regs[n].d[*] Qn =
                 * regs[n].d[1]:regs[n].d[0] Dn = regs[n].d[0] Sn = regs[n].d[0]
                 * bits 31..0 Hn = regs[n].d[0] bits 15..0
                 *
                 * This corresponds to the architecturally defined mapping
                 * between the two execution states, and means we do not need to
                 * explicitly map these registers when changing states.
                 *
...
uint64_t libqflex_read_sysreg(size_t cpu_index, uint8_t op0, uint8_t op1,
                              uint8_t op2, uint8_t crn, uint8_t crm,
                              bool ignore_permission_check)
{
        vCPU_t *cpu_wrapper = lookup_vcpu(cpu_index);

        ARMCPRegInfo const *ri =
            get_arm_cp_reginfo(cpu_wrapper->cpu->cp_regs,
                               ENCODE_AA64_CP_REG(CP_REG_ARM64_SYSREG_CP, crn,
                                                  crm, op0, op1, op2));
        ...
        if (!ignore_permission_check &&
            !cp_access_ok(arm_current_el(cpu_wrapper->env), ri, true)) {
        ...
        // Msutherl: do it the slow way by linear searching if previous encoding
        // didn't work
        for (size_t i = 0; i < cpu_wrapper->cpu->cpreg_array_len; i++) {

                uint32_t regidx =
                    kvm_to_cpreg_id(cpu_wrapper->cpu->cpreg_indexes[i]);

                ri = get_arm_cp_reginfo(cpu_wrapper->cpu->cp_regs, regidx);

                if (ri->opc0 == op0 && ri->opc1 == op1 && ri->opc2 == op2 &&
                    ri->crn == crn && ri->crm == crm &&
...
size_t libqflex_get_nb_cores(void) { return qemu_libqflex_state.n_vcpus; }
...
physical_address_t libqflex_translate_va2pa(size_t cpu_index,
                                            logical_address_t va)
{
        ...
        hwaddr pa =
            arm_cpu_get_phys_page_attrs_debug(cpu_wrapper->state, va, &attrs);

        // Return the error if there is one, otherwise cast the returned address
        return (pa == -1) ? -1 : (physical_address_t)pa;
}

logical_address_t libqflex_get_pc(size_t cpu_index)
{
        vCPU_t *cpu_wrapper = lookup_vcpu(cpu_index);
        return cpu_wrapper->env->pc;
}
...
bool libqflex_has_interrupt(size_t cpu_index)
{
        ...
        vCPU_t *cpu_wrapper = lookup_vcpu(cpu_index);
        return cpu_wrapper->cc->tcg_ops->cpu_check_interrupt(
            cpu_wrapper->state, cpu_wrapper->state->interrupt_request);
}
...
uint64_t libqflex_advance(size_t cpu_index, bool trigger_count)
{
        vCPU_t *cpu_wrapper = lookup_vcpu(cpu_index);
        if (trigger_count)
