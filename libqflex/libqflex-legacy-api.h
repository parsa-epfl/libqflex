...
typedef enum { QEMU_Instruction_Cache = 1, QEMU_Data_Cache = 2 } cache_type_t;
...
typedef struct {
        ...
        uint8_t data_is_set_and_way : 1; // wether or not the operation provides
                                         // set&way or address (range)

        union {
                set_and_way_data_t set_and_way;
                address_range_t addr_range; // same start and end addresses for
                                            // not range operations
        };
...
// typedef uint64_t          (*QEMU_GET_CSR_t)        (conf_object_t *cpu, int
// idx); typedef uint64_t          (*QEMU_GET_CYCLES_LEFT_t)(void); typedef
// uint64_t          (*QEMU_GET_FPR_t)        (conf_object_t *cpu, int idx);
// typedef uint64_t          (*QEMU_GET_GPR_t)        (conf_object_t *cpu, int
// idx); typedef conf_object_t    *(*QEMU_GET_OBJ_BY_NAME_t)(const char *name);
// typedef int               (*QEMU_GET_PL_t)         (conf_object_t *cpu);
// typedef char             *(*QEMU_GET_SNAP_t)       (conf_object_t* cpu);
// typedef int               (*QEMU_MEM_OP_IS_DATA_t) (generic_transaction_t
// *mop); typedef int (*QEMU_MEM_OP_IS_WRITE_t)(generic_transaction_t *mop);

typedef physical_address_t (*QEMU_GET_PA_t)(size_t core_index,
                                            logical_address_t va);
typedef uint64_t (*QEMU_READ_REG_t)(size_t core_index, register_type_t reg,
                                    size_t reg_info);
typedef uint64_t (*QEMU_READ_SYSREG_t)(size_t core_index, uint8_t op0,
                                       uint8_t op1, uint8_t op2, uint8_t crn,
                                       uint8_t crm,
...
typedef void (*QEMU_GET_MEM_t)(uint8_t *buffer, physical_address_t pa,
                               size_t nb_bytes);
