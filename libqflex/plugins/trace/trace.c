 ...
 */
static void trans_free(gpointer data)
{
 ...
 */
static void dispatch_memory_access(unsigned int vcpu_index,
                                   ...
                                   void *userdata)
{
         ...
         * Usless possibly, only useful to retrieve info about atomic store or
         * load
         */
 ...
 * @param userdata Generic translation info.
 */
static void dispatch_instruction(unsigned int vcpu_index, void *userdata)
{
        ...
        tr.s.physical_address = arm_cpu_get_phys_page_attrs_debug(
            current_cpu, insn->target_pc_va, &attrs);
        tr.s.exception = insn->exception_lvl;

        tr.s.size = insn->byte_size;
        tr.s.branch_type = decode_armv8_branch_opcode(&br_type, insn->opcode)
                               ? br_type
                               : QEMU_Non_Branch;
 ...
 */
static void dispatch_vcpu_tb_trans(qemu_plugin_id_t id,
                                   struct qemu_plugin_tb *tb)
{
        ...
        for (size_t i = 0; i < nb_instruction; i++) {
                ...
                g_mutex_lock(&lock);
                transaction =
                    g_hash_table_lookup(tb_table, GSIZE_TO_POINTER(host_pc_pa));
                g_mutex_unlock(&lock);

                if (transaction == NULL) {
                        transaction = g_new0(trace_insn_t, 1);

                        transaction->target_pc_va =
                            qemu_plugin_insn_vaddr(insn);
                        transaction->opcode =
                            *(uint32_t *)qemu_plugin_insn_haddr(
                                insn); //? From the official plugins
                        transaction->byte_size = qemu_plugin_insn_size(insn);
                        transaction->disas_str = qemu_plugin_insn_disas(insn);
                        transaction->exception_lvl =
                            arm_current_el(&ARM_CPU(current_cpu)->env);

                        g_mutex_lock(&lock);
                        g_hash_table_insert(tb_table,
                                            GSIZE_TO_POINTER(host_pc_pa),
                                            transaction);
                        g_mutex_unlock(&lock);
                }

                qemu_plugin_register_vcpu_mem_cb(
                    insn, dispatch_memory_access, QEMU_PLUGIN_CB_NO_REGS,
                    QEMU_PLUGIN_MEM_RW, (void *)transaction);

                qemu_plugin_register_vcpu_insn_exec_cb(
                    insn, dispatch_instruction, QEMU_PLUGIN_CB_NO_REGS,
                    (void *)transaction);
        }
}

static void exit_plugin(qemu_plugin_id_t id, void *p)
{
        ...
        char *size_logger =
            g_strdup_printf("> HASH_MAP_SIZE: %i\n", hashmap_size);
        char *space_logger =
            g_strdup_printf("> HASH_MAP_MBYTES: %f\n", hashmap_M_space);
        char *struct_size =
            g_strdup_printf("> TRANSLATION_BYTES: %li\n", sizeof(trace_insn_t));
...
/**
 * Plugin entry. Parse the arguments. Register the call back for each
 * transaction, as well as plugin exit.
 */
void libqflex_trace_init(void)
{
        ...
        tb_table =
            g_hash_table_new_full(NULL, g_direct_equal, NULL, trans_free);

        // Register translation callback
        qemu_plugin_register_vcpu_tb_trans_cb(qflex_trace_id,
                                              dispatch_vcpu_tb_trans);
