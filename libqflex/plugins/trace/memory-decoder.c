...
static bool disas_ldst_ldapr_stlr(struct mem_access *s, uint32_t opcode)
{
        ...
        if (is_store) {
                // do_gpr_st(s, cpu_reg(s, rt), clean_addr, mop, true, rt,
                // iss_sf, true);
        } else {
    ...
    if (is_mult) {
        if (is_zero) {
            if (s->ata) {
                ...
                int log2_dcz_bytes = 7 + 2; // env_archcpu(env)->dcz_blocksize +
2; int log2_tag_bytes = log2_dcz_bytes - (LOG2_TAG_GRANULE + 1); intptr_t
tag_bytes = (intptr_t)1 << log2_tag_bytes; memset(mem, tag_pair, tag_bytes);
            }
...
static bool disas_ldst_single_struct(struct mem_access *s, uint32_t opcode)
{
        ...
        for (xs = 0; xs < selem; xs++) {
                if (replicate) {
                        ...
                        // do_dup(scale, vec_full_reg_offset(s, rt), (is_q + 1)
                        // * 8, vec_full_reg_size(s), tcg_tmp, NULL, 0);
                } else {
...
static bool disas_ldst_pair(struct mem_access *s, uint32_t opcode)
{
        ...
        case 0:
                 ...
                 * emulate caches we don't care about hints to the cache system
                 * about data access patterns, and handle this identically to
                 * plain signed offset.
                 */
                if (is_signed) {
        ...
        if (set_tag) {
                // Unsupported
                printf("ERROR:QFlex, We do not support memory callbacks for "
                       "tags.");
                /*
                if (!s->ata) {
        ...
        *s = (struct mem_access){.size = size,
        ...
        if (is_vector) {
                ...
                if (is_load) {
                         ...
                         */
                        // do_gpr_ld(s, tmp, clean_addr, size + is_signed *
                        // MO_SIGN,
                        //           false, false, 0, false, false);
                        // do_gpr_ld(s, tcg_rt2, clean_addr, size + is_signed *
                        // MO_SIGN,
                        //           false, false, 0, false, false);
                        *s = (struct mem_access){.size = size,
                                                 .is_vector = is_vector,
...
static bool disas_ldst_excl(struct mem_access *s, uint32_t opcode)
{
        ...
        case 0x9: /* STLR */
                ...
                // do_gpr_st(s, cpu_reg(s, rt), clean_addr, size | MO_ALIGN,
                // true, rt,
                //           disas_ldst_compute_iss_sf(size, false, 0),
                //           is_lasr);
                *s = (struct mem_access){.size = size,
                                         .is_vector = false,
                                         .is_load = true,
        ...
        case 0xd: /* LDAR */
                /* Generate ISS for non-exclusive accesses including LASR.  */
                /* TODO: ARMv8.4-LSE SCTLR.nAA */
                // do_gpr_ld(s, cpu_reg(s, rt), clean_addr, size | MO_ALIGN,
                // false, true,
                //           rt, disas_ldst_compute_iss_sf(size, false, 0),
                //           is_lasr);
                *s = (struct mem_access){.size = size,
                                         .is_vector = false,
                                         .is_load = true,
        ...
        case 0x3:               /* CASP / STXP */
                if (size & 2) { /* STXP / STLXP */
                        // gen_store_exclusive(s, rs, rt, rt2, clean_addr, size,
                        // true);
                        if (size == 2) {
                                // Must access 2x32 bits in single access (64
                                // bits)
        ...
        case 0x7:               /* CASPA / LDXP */
                if (size & 2) { /* LDXP / LDAXP */
                        // addr = r[n]
                        // gen_load_exclusive(s, rt, rt2, clean_addr, size,
                        // true);
                        if (size == 2) {
                                // Must access 2x32 bits in single access (64
                                // bits)
        ...
        case 0xf: /* CASAL */
                if (rt2 ==
                    31) { // Assume dc_isar_feature(aa64_atomics, s) == true
 ...
 * S: sign for imm9.
 */
static bool disas_ldst_pac(struct mem_access *s, uint32_t opcode, int size,
                           int rt, bool is_vector)
{
        if (size != 3 ||
            is_vector) { // Assume `dc_isar_feature(aa64_pauth, s) == true`
                // unallocated_encoding(s);
                return false;
        }
...
static bool disas_ldst_reg_roffset(struct mem_access *s, uint32_t opcode,
                                   int opc, int size, int rt, bool is_vector)
{
        ...
        if (is_vector) {
                ...
                if (is_store) {
                        ...
                        // do_gpr_ld(s, tcg_rt, clean_addr, size + is_signed *
                        // MO_SIGN,
...
static bool disas_ldst_atomic(struct mem_access *s, uint32_t opcode, int size,
                              int rt, bool is_vector)
{
        int rs = extract32(opcode, 16, 5);
        ...
        if (o3_opc == 014) {
                ...
                //           true, rt, disas_ldst_compute_iss_sf(size, false,
                //           0), true);
                *s = (struct mem_access){.size = size,
                                         .is_vector = false,
                                         .is_load = true,
 ...
 * opc: 00 -> store, 01 -> loadu, 10 -> loads 64, 11 -> loads 32
 */
static bool disas_ldst_reg_imm9(struct mem_access *s, uint32_t opcode, int opc,
...
static bool disas_ldst_reg(struct mem_access *s, uint32_t opcode)
{
        ...
        case 0:
                if (extract32(opcode, 21, 1) == 0) {
                         ...
                         */
                        return disas_ldst_reg_imm9(s, opcode, opc, size, rt,
                                                   is_vector);
                }
                switch (extract32(opcode, 10, 2)) {
                case 0:
                        return disas_ldst_atomic(s, opcode, size, rt,
                                                 is_vector);
                case 2:
                        return disas_ldst_reg_roffset(s, opcode, opc, size, rt,
                                                      is_vector);
                default:
                ...
                break;
        case 1:
                return disas_ldst_reg_unsigned_imm(s, opcode, opc, size, rt,
                                                   is_vector);
        }
        // unallocated_encoding(s);
        return false;
...
static bool disas_ldst(struct mem_access *s, uint32_t opcode)
{
        ...
        case 0x19:
                if (extract32(opcode, 21, 1) != 0) {
                        printf("ERROR:QFlex, We do not support memory "
                               "callbacks for tags.\n");
...
static bool disas_branch_sys(struct mem_access *s, uint32_t opcode)
{
        bool has_mem_access = false;
        ...
        case 0x6: /* Exception handling and system instructions */
                ...
                case 0x1:
                        ...
                        case 0x32: /* Hints to compiler and special instructions
                                      like NOPs (TODO: may need to be checked in
                                      detail) */
                        case 0x33: /* Barriers */
                        case 0x4:
                        case 0x14:
                        ...
                        case 0x54:
                        case 0x64:
                        case 0x74: /* For controlling processor flags */
                                break;
                        default:
                                ...
                                case 0x4: /* Transcation start and end */
                                        break;
                                case 0x1:
                                case 0x5: /* System instructions */
                                        switch (extract32(opcode, 12, 4)) {
                                        case 0x7: /* Data Cache and Instr. Cache
                                                     operation */
                                                switch (extract32(
                                                    opcode, 8,
                                                    4)) { /* CRm field */
 ...
 */
static bool disas_sve(struct mem_access *s, uint32_t opcode)
{
        bool has_mem_access = false;
        // Only handling these three fields as others not used
        size_t size = 0; // 10 for 32-bit and 11 for 64-bit
        bool is_atomic =
            false; // Is atomic instruction? False for all SVE memory accesses
        bool is_store = false; // Is load/store instruction?

        switch (extract32(opcode, 29, 3)) {
        case 0x4:
                has_mem_access = true; /* SVE Memory - 32-bit Gather and Unsized
                                          Contiguous */
                ...
                case 0x3: /* LDR, and SVE prefetch */
                        ...
                        case 0x1: /* SVE prefetch (scalar + immediate) */
                                ...
                                break;
                        default:
                                size = 3; /* 32-bit (TODO: How to set this for
                                             vector register?) */
                                break;
                        }
                        break;
                default:
                        ...
                        case 0x6: /* SVE prefetch (scalar + scalar) */
                                ...
                                break;
                        default:
                                if (extract32(opcode, 22, 1) == 0x1 &&
                                    extract32(opcode, 15, 1) ==
                                        0x1) { /* SVE load and broadcast*/
                                        switch (extract32(opcode, 23, 2)) {
                                        case 0x0:
                                                size = extract32(opcode, 13, 2);
                                                break;
                                        case 0x1:
                                                size = (extract32(opcode, 13,
                                                                  2) == 0x0)
                                                           ? 2
                                                           : extract32(opcode,
                                                                       13, 2);
        ...
        case 0x5:
                ...
                case 0x5:
                        ...
                        case 0x0:
                                ...
                                break;
                        case 0x1:
                                size = (extract32(opcode, 21, 2) == 0x0)
                                           ? 2
                                           : extract32(opcode, 21, 2);
                                break;
                        case 0x2:
                ...
                case 0x7:
                        ...
                        case 0x0:
                                if (extract32(opcode, 20, 1) == 0x0)
                                        ...
                                        size = 4;
                                        printf(
                                            "[Warn]:QFlex, loading quadwords, "
                                            "setting size_t = 4.\n");
                                }
                                break;
                        default:
                                g_assert(
                                    extract32(opcode, 20, 1) ==
                                    0x0); /* The other one is unallocated */
                                size = extract32(opcode, 23, 2);
                                break;
                        }
                        break;
                case 0x4:
                        ...
                        default:
                                printf(
                                    "[Error]:QFlex, Unallocated encoding.\n");
                                g_assert_not_reached();
                                break;
                        }
                        break;
        ...
        case 0x7:
                ...
                case 0x1:
                        if (extract32(opcode, 21, 1) == 0x1) {
                                size = 4;
                                printf("[Warn]:QFlex, loading quadwords, "
                                       "setting size_t = 4.\n");
                        } else {
                                size =
                                    (extract32(opcode, 22, 1) == 0x0) ? 3 : 2;
                        }
                        break;
                case 0x3:
                case 0x4:
...
bool decode_armv8_mem_opcode(struct mem_access *s, uint32_t opcode)
{
        ...
        case 0x9: /* Data processing - immediate */
                break;
        case 0xa:
        case 0xb: /* Branch, exception generation and system opcodes */
