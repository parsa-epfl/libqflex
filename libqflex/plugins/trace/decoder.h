 ...
 * Rm (post-index only): post-index register (when !31) or size dependent #imm
 */
static bool disas_ldst_multiple_struct(struct struct mem_access* s, uint32_t opcode)
{
    int rm = extract32(opcode, 16, 5);
    int size = extract32(opcode, 10, 2);
    int opcode = extract32(opcode, 12, 4);
    ...
    *s = (mem_access) {.size = size,
          ...
          .is_signed = false,
          .is_pair = false,
          .accesses = rpt * elements mem_access* selem};
    return true;
}
 ...
 * opc: 00 -> store, 01 -> loadu, 10 -> loads 64, 11 -> loads 32
 */
static bool disas_ldst_reg_imm9(struct struct mem_access* s, uint32_t opcode,
                                int opc,
                                int size,
                                int rt,
                                bool is_vector)
 ...
 * Rt: target register
 */
static bool disas_ldst_reg_unsigned_imm(struct struct mem_access* s, uint32_t opcode,
                                        int opc,
                                        int size,
                                        int rt,
                                        bool is_vector)
...
static bool disas_ldst_reg(struct struct mem_access* s, uint32_t opcode)
{
    int rt = extract32(opcode, 0, 5);
    int opc = extract32(opcode, 22, 2);
    bool is_vector = extract32(opcode, 26, 1);
...
bool decode_memory_insn(struct mem_access* s, uint32_t opcode)
{
